...
Settings 的 **Export standalone application / 导出独立应用** 以“整个记号文件”为选择单位。文件注册的主记号、分析记号、分类和生成类会一起打包；两个 shared 文件与通过记号 ID 检测到的内置依赖自动加入，不单独显示为可选项。本地列表只包含当前已启用、已信任、源码版本一致且加载成功的文件。
...
独立应用保留展开、分析、笔记、LaTeX、等价表示、Tools 与 xlsx 导入导出，但记号文件集合固定：本地文件管理器替换为只读清单。导出器按所选内置记号反查 `src/notations` 源文件并嵌入；同一个生成簇的多个变体只嵌入一次生成器源文件。本地文件显示为 `Local/<name>`。清单中的每个内置或本地记号文件都可以下载导出时嵌入的源码副本；下载不会修改应用状态，也不会把源码上传到远端。浏览器下载不能创建原目录，因此下载名会把 `/` 或 `\` 替换成 `__`，并在同名时追加编号。产物仍不包含编辑、重新上传、启用/禁用、删除或再次导出记号文件的入口。共享 helper 文件属于运行时内部资源，不作为用户可下载的记号文件显示。
