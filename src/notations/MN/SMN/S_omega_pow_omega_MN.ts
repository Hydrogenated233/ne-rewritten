...
function sep_cantor_display(s: Sep): string {
    const result: string[] = [];
...
function vertical_cantor_display(v: Vertical): string {
...
function compute_mountain_diagram(expr: Mountain, current_equiv?: string): MountainDiagramData | undefined {
    ...
    const sorted_verticals = sorted.map(vertical_cantor_display);
...
const draw_diagram_control: DiagramControl<Mountain, DiagramData> = {
    ...
    draw_diagram: (_expr, _data) => {
        ...
        return draw_mountain_diagram(mountain, {
            invert_vertical: _data.invert_vertical ?? false,
...
export const S_omega_pow_omega_MN: NotationDefinition<Mountain> = {
    id: 'S-omega^omega-MN',
    name: "Smile's ω^ω MN",
    simple_name: 'Sω^ωMN',
