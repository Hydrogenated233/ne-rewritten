...
import { DEN2, weak_DEN2 } from '@/notations/DEN/DEN2.ts';
...
import { category_n_mn } from '@/notations/MN/SMN/n_MN.ts';
import { SA_omega2_MN } from '@/notations/MN/SMN/SA_omega2_MN.ts';
import { S_omega2_MN } from '@/notations/MN/SMN/S_omega2_MN.ts';
import { S_omega_pow_omega_MN } from '@/notations/MN/SMN/S_omega_pow_omega_MN.ts';
...
import { category_upmn, category_upmn_test } from '@/notations/MN/UPMN/categories.ts';
...
if (IS_STANDALONE) {
    const allowed = new Set((window as any).__NE_STANDALONE_VARIANT_IDS__ ?? []);
    settings.variant_state = Object.fromEntries(
        Object.entries(settings.variant_state ?? {}).map(([base, entries]) =>
            [base, Array.isArray(entries) ? entries.filter((entry) => allowed.has(`${base}$${entry.seq}`)) : []]),
    );
}
...
register_category(category_upmn_test);
...
if (!get_notation(settings.current_notation_id)) {
    settings.current_notation_id = list_notations()[0]?.id ?? '';
}
