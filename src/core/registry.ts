...
export function register_category(cat: NotationCategoryDefinition): void {
    if (category_defs.has(cat.id) || map.has(cat.id) || variant_base_of.has(cat.id)) {
    ...
    if (cat.generator) {
        ...
        const previous_state = { ...gen_state };
        try {
            register_generated_members(cat);
        } catch (error) {
            unregister_category(cat.id);
            gen_state = previous_state;
            throw error;
        }
    }
}
 ...
 */
function _register_notation<T>(notation: NotationDefinition<T>, after_id?: string, variant = false): void {
    if (!variant && variant_base_of.has(notation.id)) {
        throw new RegisterError(notation.id, `Id '${notation.id}' is reserved by an initial variant.`);
    }
...
function notation_count_key(id: string): string {
    id = get_init_variant_meta(id)?.base_id ?? id;
...
/** Count generated families and base/initial-variant families as one logical notation each. */
...
// 删除: core 注销并移除定义, UI 生命周期同步清理树、分析、笔记与设置, 避免编号回填继承残留。
...
/** Include dormant definitions as well as live variants when managing a base file. */
export function with_init_variant_ids(ids: Iterable<string>): string[] {
    return [...new Set(Array.from(ids).flatMap((id) => [id, ...list_init_variant_ids(id)]))];
}
...
export function set_variant_state(state: Record<string, InitVariantDef[]>): void {
    for (const id of variant_base_of.keys()) unregister_notation_raw(id);
    variant_state.clear();
    for (const base_id of Object.keys(state ?? {})) {
        ...
        for (const e of list) {
            if (e && Number.isSafeInteger(e.seq) && e.seq >= 1 && Array.isArray(e.init) &&
                e.init.length > 0 && e.init.every((value: unknown) => typeof value === 'string') &&
                !entries.some((entry) => entry.seq === e.seq)) {
...
function parse_init_list(base: NotationDefinition<any>, strings: string[]): any[] | null {
    if (!Array.isArray(strings) || strings.length === 0 || strings.some((s) => typeof s !== 'string')) return null;
...
function ensure_variants(base_id: string): void {
    if (variant_base_of.has(base_id)) return;
    const entries = variant_state.get(base_id);
    ...
    for (const entry of entries) {
        ...
        try {
            _register_notation(build_init_variant_definition(base, id, parsed), anchor, true);
        } catch (err) {
...
export interface InitVariantCreateResult {
    ok: boolean;
    id?: string;
    error?: 'unknown-base' | 'no-from-display' | 'generator-base' | 'variant-base' | 'parse' | 'id-conflict';
}
...
export function create_init_variant(base_id: string, init_strings: string[]): InitVariantCreateResult {
    if (is_init_variant(base_id)) return { ok: false, error: 'variant-base' };
    const base = map.get(base_id);
    ...
    if (map.has(id) || category_defs.has(id) || variant_base_of.has(id)) return { ok: false, error: 'id-conflict' };
...
export function remove_init_variant(id: string): void {
    ...
    if (!entries) return;
    const remaining = entries.filter((entry) => entry.seq !== meta.seq);
    if (remaining.length) variant_state.set(meta.base_id, remaining);
    else variant_state.delete(meta.base_id);
    rebuild_variant_index();
